...
internal sealed class RouteTree {
    ...
    public Route? Find(Method method, string[] parts) => Search(Root, parts, 0, method);

    private static Route? Search(Node node, string[] parts, int index, Method method) {
        ...
        if (node.TryGetStatic(part, out var staticChild)) {
            var hit = Search(staticChild, parts, index + 1, method);
            if (hit is not null)
                return hit;
        }

        if (node.ParamNode is not null) {
            var hit = Search(node.ParamNode, parts, index + 1, method);
            if (hit is not null)
                return hit;
        }
    ...
    private sealed class Node {
        ...
        public bool TryGetStatic(string key, out Node child) {
            if (StaticChildren is not null)
                return StaticChildren.TryGetValue(key, out child!);
            child = null!;
            return false;
        }
