...
const ctxWith = (star: string, url = 'http://localhost/x'): Context => {
    const req = {
        url,
        method: 'GET',
        headers: new Headers(),
    } as unknown as Request;
    return new Context(req, new MatchedRoute({} as never, { '*': star }));
};
...
describe('assetHandler', () => {
    it('serves a file inside the dir', async () => {
        const res = await assetHandler({ dir })(ctxWith('hello.txt'));
        expect(res.status).toBe(200);
        expect(await res.text()).toBe('hello world');
    });

    it('serves nested files', async () => {
        const res = await assetHandler({ dir })(ctxWith('sub/nested.txt'));
        expect(await res.text()).toBe('nested');
    });

    it('404s for a missing file', async () => {
        const res = await assetHandler({ dir })(ctxWith('nope.txt'));
        expect(res.status).toBe(404);
    });

    it('blocks path traversal out of the dir', async () => {
        const res = await assetHandler({ dir })(ctxWith('../../etc/passwd'));
        expect(res.status).toBe(404);
    });
    ...
    it('serves immutable when the request carries ?v= (busted url)', async () => {
        const res = await assetHandler({ dir })(
            ctxWith('hello.txt', 'http://localhost/static/hello.txt?v=abc'),
        );
        expect(res.headers.get('cache-control')).toBe(
    ...
    it('does NOT serve immutable for a bare request (no ?v=)', async () => {
        const res = await assetHandler({ dir, maxAge: 60 })(
            ctxWith('hello.txt', 'http://localhost/static/hello.txt'),
        );
