...
describe('Router dispatch', () => {
    it('returns an html response', async () => {
        ...
        router.register(Method.Get, '/hello', () => Reply.html('hi there'));
    ...
    it('returns a json response', async () => {
        const router = new Router();
        router.register(Method.Get, '/data', () =>
            Reply.json({ ok: true, n: 1 }),
        );
    ...
    it('applies a chosen status', async () => {
        const router = new Router();
        router.register(Method.Get, '/made', () =>
            Reply.text('made').status(201),
        );
    ...
    it('passes a native Response through untouched', async () => {
        ...
        router.register(Method.Get, '/raw', () => sentinel);
    ...
    it('awaits an async handler', async () => {
        const router = new Router();
        router.register(Method.Get, '/async', async () => {
    ...
    it('405s a method mismatch with an Allow header', async () => {
        const router = new Router();
        router.register(Method.Post, '/hello', () => Reply.text('hi'));
        router.register(Method.Put, '/hello', () => Reply.text('hi'));
...
describe('dynamic params', () => {
    it('captures a single param into the context', async () => {
        const router = new Router();
        router.register(Method.Get, '/hello/:id', (ctx) =>
    ...
    it('captures multiple params', async () => {
        const router = new Router();
        router.register(Method.Get, '/u/:user/post/:post', (ctx) =>
    ...
    it('captures adjacent params', async () => {
        const router = new Router();
        router.register(Method.Get, '/:year/:month/:day', (ctx) =>
            Reply.text(
    ...
    it('captures params bookended by static segments', async () => {
        const router = new Router();
        router.register(Method.Get, '/api/:version/users/:id/posts', (ctx) =>
    ...
    it('decodes each param independently', async () => {
        const router = new Router();
        router.register(Method.Get, '/:a/:b', (ctx) =>
    ...
    it('decodes percent-encoded segments', async () => {
        const router = new Router();
        router.register(Method.Get, '/hello/:name', (ctx) =>
    ...
    it('does not match when the segment count differs', async () => {
        const router = new Router();
        router.register(Method.Get, '/hello/:id', () => Reply.text('hit'));
...
describe('static beats param', () => {
    it('prefers a static segment when the param is registered first', async () => {
        const router = new Router();
        router.register(Method.Get, '/test/:id', (ctx) =>
            Reply.text(`id=${ctx.params.id}`),
        );
        router.register(Method.Get, '/test/new', () => Reply.text('new'));
...
describe('registration order', () => {
    it('serves the first defined route on an identical static collision', async () => {
        const router = new Router();
        router.register(Method.Get, '/test/new', () => Reply.text('first'));
        router.register(Method.Get, '/test/new', () => Reply.text('second'));
    ...
    it('serves the first defined route on an identical param collision', async () => {
        const router = new Router();
        router.register(Method.Get, '/test/:id', () => Reply.text('first'));
        router.register(Method.Get, '/test/:slug', () => Reply.text('second'));
...
describe('Request', () => {
    it('passes the request through and exposes a param accessor', async () => {
        ...
        router.register(Method.Get, '/x/:id', (ctx) => {
            seen = ctx;
