...
describe('wildcard route matching', () => {
    it('matches the base and captures the rest under *', () => {
        ...
        routes.add(Method.Get, '/home/*', noop);
        expect(routes.match(Method.Get, parts('/home/a'))?.params['*']).toBe(
            'a',
        );
        expect(
            routes.match(Method.Get, parts('/home/a/b/c'))?.params['*'],
        ).toBe('a/b/c');
        expect(routes.match(Method.Get, parts('/home'))?.params['*']).toBe('');
    });

    it('does not match a different prefix', () => {
        const routes = new RouteCollection();
        routes.add(Method.Get, '/home/*', noop);
        expect(routes.match(Method.Get, parts('/about/x'))).toBeNull();
    });

    it('prefers an exact route over the wildcard', () => {
        const routes = new RouteCollection();
        routes.add(Method.Get, '/home/*', noop);
        const exact = routes.add(Method.Get, '/home/settings', noop);
        expect(routes.match(Method.Get, parts('/home/settings'))?.route).toBe(
            exact,
        );
    });
    ...
    it('prefers a param route over the wildcard', () => {
        const routes = new RouteCollection();
        routes.add(Method.Get, '/home/*', noop);
        const param = routes.add(Method.Get, '/home/:id', noop);
        const match = routes.match(Method.Get, parts('/home/42'));
    ...
    it('decodes wildcard segments', () => {
        const routes = new RouteCollection();
        routes.add(Method.Get, '/files/*', noop);
        expect(
            routes.match(Method.Get, parts('/files/a%20b/c'))?.params['*'],
        ).toBe('a b/c');
    });
});
...
describe('wildcard combined with route params', () => {
    it('captures named params and the wildcard tail together', () => {
        const routes = new RouteCollection();
        routes.add(Method.Get, '/users/:id/files/*', noop);
    ...
    it('handles a wildcard immediately after a param', () => {
        const routes = new RouteCollection();
        routes.add(Method.Get, '/users/:id/*', noop);
        const match = routes.match(Method.Get, parts('/users/7/a/b'));
    ...
    it('captures multiple params before the wildcard', () => {
        const routes = new RouteCollection();
        routes.add(Method.Get, '/:org/:repo/tree/*', noop);
        const match = routes.match(
            Method.Get,
