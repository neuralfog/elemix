...
public sealed class RouterTests {
    ...
    [Fact]
    public void WildcardMatchesPathsDeeperThanTheStackBuffer() {
        var router = new Router();
        router.Get("/files/*", Ok);
        ...
        for (var i = 0; i < parts.Length; i++)
            parts[i] = i.ToString();
        var deep = string.Join('/', parts);

        Assert.Equal(deep, router.Match(Method.Get, $"/files/{deep}")!.Param("*"));
    }

    [Fact]
    public void StaticRoutesMatchDeeperThanTheStackBuffer() {
        var router = new Router();
        var parts = new string[40];
        for (var i = 0; i < parts.Length; i++)
            parts[i] = $"s{i}";
        var path = $"/{string.Join('/', parts)}";
        router.Get(path, Ok);

        Assert.NotNull(router.Match(Method.Get, path));
        Assert.Null(router.Match(Method.Get, $"{path}/extra"));
    }
}
