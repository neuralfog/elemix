...
- [ ] Put a website at `elemix.dev`
  - [ ] Rewrite elemix.dev with ssr - hydris
  - [x] Landing page
...
- Templates for SSR are generated by the compiler - AOT - the render pipeline becomes string concatenation and value interpolation
  can't get any faster than that 🚤💨 Half the pain online comes from async render trees 🤮
- Server components that fetch inside the render tree are a mess - so NO `getServerSideProps` or `useAsyncData` garbage 🤮
- Structural takeover hydration 100% on the client side, no flashing or wasteful rerenders, server-rendered tree gets adopted 💙⚙️
- Components are views that can be returned from `handler/controller`, old school - a model that has been working for decades. This
  also comes with the ability to compose applications without restriction - this is not a toy 🪁 to build landing pages 🤓

```ts

return Reply.view(PageComponent)

```

- Components have the ability to receive `viewData` and use it for rendering 💙 `viewData` can be used in either SSR or CSR 🧑‍💻

```ts
const data = await Promise.all([...])
return Reply.view(PageComponent, data)
```

- New compiler hint to mark client-only components `// #client`, all children of a client component will naturally be rendered on the client
  side only 🌐
- Real shared state across a CSR/SSR boundary 💪💪
- Stores are module-level variables in a long-running process here, so global state is scoped and isolated per request 🚧
- Stores can be super seeded with `viewData` just like components 🌱
- `// #before-mount` fires server-side too, during the render pipeline ⚙️ It must stay synchronous - no async work in `// #before-mount` 🚫🔄
- It just works 🎉🎉🎉
